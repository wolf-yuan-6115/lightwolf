...
import dev.wolf.yuan.pumper.protocol.EqBand
import dev.wolf.yuan.pumper.protocol.FilterType
import dev.wolf.yuan.pumper.protocol.WidthMode
    ...
    Column(
        ...
        state.config.bands.forEachIndexed { index, band ->
            ...
            SegmentedListItem(
                ...
                supportingContent = {
                    FlowRow(
                        horizontalArrangement = Arrangement.spacedBy(PumperSpacing.extraSmall),
                        verticalArrangement = Arrangement.spacedBy(PumperSpacing.extraSmall),
                    ) {
                        BandDetail(filterLabel(band.type))
                        BandDetail(formatFrequency(band.frequencyHz))
                        if (!band.type.isGainIndependent()) BandDetail("${formatSigned(band.gainDb)} dB")
                        if (!band.enabled) BandDetail("Off")
                    }
                },
...
@Composable
private fun BandDetail(value: String) {
    Surface(
