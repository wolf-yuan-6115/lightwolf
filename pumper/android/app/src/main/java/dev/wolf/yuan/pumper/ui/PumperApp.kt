...
import dev.wolf.yuan.pumper.AppDestination
import dev.wolf.yuan.pumper.Confirmation
import dev.wolf.yuan.pumper.ConnectionState
import dev.wolf.yuan.pumper.ControllerUiState
import dev.wolf.yuan.pumper.DebugFeatures
import dev.wolf.yuan.pumper.PumperControllerViewModel
import dev.wolf.yuan.pumper.settings.ThemeMode
...
private val Destinations = listOf(
    ...
    DestinationItem(
        ...
        "Audio",
        "Audio",
        "Playback and headphone controls",
...
fun PumperApp(
    ...
    val destinationItem = Destinations.first { it.destination == state.destination }
    val subtitleAlpha = (1f - appBarScrollBehavior.state.collapsedFraction * 2f).coerceIn(0f, 1f)
            ...
            topBar = {
                if (state.connected) {
                    LargeFlexibleTopAppBar(
                        title = { Text(destinationItem.title) },
                        subtitle = {
                            Text(destinationItem.subtitle, modifier = Modifier.alpha(subtitleAlpha))
                        },
