...
The firmware enumerates as a composite USB Audio Class 2 and vendor HID device. Core 0 services TinyUSB and core 1 processes the ten-band EQ before audio enters the PIO/DMA I2S ring. Pre-EQ input and saturated post-EQ output stereo peak/RMS metering is streamed over HID only while the controller's heartbeat remains active. Device status also reports the RP2350's approximate junction temperature.
