...
int main(void) {
  ...
  assert(hid_activity_level(&s, 25099, false));
  assert(!hid_activity_level(&s, 25100, false));
  assert(!hid_activity_level(&s, 50099, false));
  assert(hid_activity_level(&s, 50100, false));
  assert(!hid_activity_level(&s, 75100, false));
  assert(!hid_activity_level(&s, 100100, false));
  assert(s.phase == 0);
  ...
  for (uint32_t t = 0; t < 1000000; t += 5000) {
    ...
    assert(hid_activity_level(&s, t, false) == ((t / 25000) % 2 == 0));
  }
  hid_activity_reset(&s);
  hid_activity_notify(&s, UINT32_MAX - 20000);
  assert(hid_activity_level(&s, 4000, false));
  assert(!hid_activity_level(&s, 5000, false));
  // Streaming changes change the baseline, never cancel the active pulse.
  assert(hid_activity_level(&s, 5001, true));
  hid_activity_reset(&s);
