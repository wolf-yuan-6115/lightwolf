...
static bank_header_t read_bank_header(uint8_t bank) {
  ...
  if (eq_protocol_read_u32(header) != PROFILE_BANK_MAGIC ||
      ...
      eq_protocol_read_u32(header + 16u) != crc32(header, 16u)) {
    return result;
  }
...
static bool read_profile(uint8_t bank, uint8_t index, profile_slot_t *slot) {
  ...
  if (eq_protocol_read_u32(record) != PROFILE_RECORD_MAGIC ||
      eq_protocol_read_u16(record + 4u) !=
          eq_protocol_read_u16(flash_pointer(bank_flash_offset(bank)) + 4u) ||
      record[6] != index || record[7] != PROFILE_PRESENT_MARKER ||
      eq_protocol_read_u16(record + 12u) != SETTINGS_CONFIG_SIZE ||
      eq_protocol_read_u32(record + 16u) !=
          crc32(record + PROFILE_CONFIG_OFFSET, SETTINGS_CONFIG_SIZE)) {
    return false;
  }
  eq_config_t config;
...
static bool load_profile_bank(void) {
  ...
  for (uint8_t bank = 0u; bank < PROFILE_BANK_COUNT; bank++) {
    headers[bank] = read_bank_header(bank);
    // A committed bank is accepted only as a complete configuration.
    for (uint8_t index = 0u; headers[bank].valid && index < EQ_PROFILE_COUNT; index++) {
      profile_slot_t slot;
      if ((headers[bank].present_mask & (1u << index)) && !read_profile(bank, index, &slot))
        headers[bank].valid = false;
    }
  }
...
static void build_bank_image(uint8_t default_profile, uint32_t bank_generation) {
  ...
  uint8_t *crossfeed = s_flash_image + CROSSFEED_PAGE_OFFSET;
...
bool eq_settings_save_crossfeed(crossfeed_config_t const *config) {
  if (!crossfeed_validate(config))
    return false;
  crossfeed_config_t previous = s_crossfeed;
  s_crossfeed = *config;
